"""
tests/V6_5/test_mcp_search_tool_mcp4.py
...
def main():
    ...
    try:
        ...
        rows_without_source = tool.search(
            TEST_QUERY
        )

        rows_with_source = tool.search(
            TEST_QUERY,
            source="TSMC"
        )
        ...
        # SearchTool 本身沒有 Document ID Retrieval。
        # MCP-3 Article Retrieval 使用 ArticleTool。
        #
        # 因此 MCP-4.3 SearchTool 不應該在這裡
        # 重複測試 Article Retrieval。
        #
        # 本測試改為確認 SearchTool
        # 不提供不必要的 Retrieval API。

        test(
            "SearchTool exposes search only",
            hasattr(
                tool,
                "search"
            )
        ...
        test(
            "SearchTool does not expose URL retrieval",
            not hasattr(
                tool,
                "get_by_url"
            )
        ...
        # SearchTool 是 MCP Search Tool，
        # Count 屬於 Service / Repository 能力，
        # 不應由 SearchTool 暴露。

        test(
            "SearchTool does not expose count",
            not hasattr(
                tool,
                "count"
            )
    ...
    print(
        "MCP-4.3 SearchTool Test Summary"
    )
    print(
        "=========================================="
    )
